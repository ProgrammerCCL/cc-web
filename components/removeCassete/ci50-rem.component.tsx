...
import { TMcStateRemoveCI50, STATUS_MSG_CI50 } from "@/constants/removeCassete";

interface ICI50REMProps {
  wipId: number | null;
  transId: string | null;
...
export const CI50REM = (props: ICI50REMProps) => {
  const {
    wipId,
    transId,
    setIsOpen,
  ...
  const handlerUnlockCassetNote = () => {
    if (statusMachine === "NotCasset") {
      setStatusMachine("NotCasset1");
    } else if (statusMachine === "NoteIncasset") {
    ...
    if (RemProcessData) {
      ...
      if (
        checkRemoveMcStatus({
          ...
          state: statusMachine as TMcStateRemoveCI50,
      ...
      else if (statusMachine === "NotCasset") {
        setTimeout(() => {
          setStatusMachine("NotCasset1");
        }, 1500);
      } else if (
  ...
  return (
    <div className="relative mx-auto flex h-[570px] w-4/5 -translate-y-10 flex-col items-center justify-around rounded-xl border-2 border-blue-900 bg-white shadow-md">
      <div className="mx-auto flex w-5/6 items-center justify-center rounded-lg bg-white p-4 text-center text-5xl">
        <span>
          {STATUS_MSG_CI50[statusMachine as TMcStateRemoveCI50] || ""}
        </span>
      </div>
      ...
      {/* ปุ่ม "Coin" และ "Note" */}
      {["remove", "CoinSuccess", "CoinIncasset", "NoteSuccess"].includes(
        statusMachine
      ) && (
        <div className="mb-4 flex w-5/6 justify-center space-x-20 text-2xl">
          ...
          {!isRemoveNote && !["CoinIncasset"].includes(statusMachine) && (
            <button
              onClick={() => handlerUnlockCassetNote}
      ...
      {["CoinSuccess", "NoteSuccess"].includes(statusMachine) && (
        <div className="flex w-5/6 justify-center text-2xl">
          <button
